...
3. Group items by primary_supplier_id and call create_draft_po with justified quantities
   from reorder_quantity / forecast data (never invent stock counts). Each item MUST
   include inventory_item_id, item_name (copy from list_low_stock), and quantity.
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for store in stores:
            ...
            for supplier_id, items in supplier_items.items():
                ...
                is_new, _prior = check_or_claim(idem_key, {"supplier_id": supplier_id})
                if not is_new:
                    # Same store/supplier already drafted this hour — skip to avoid
                    # flooding the approval queue with duplicate POs on repeated runs.
                    continue

                po_items = [
                    {
                        "inventoryItemId": item["id"],
                        "itemName": item.get("itemName") or item.get("item_name") or "Unknown",
